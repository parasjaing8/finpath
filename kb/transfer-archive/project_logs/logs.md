...
## 2026-04-22 — Qwen3.5 v2: 9-stage refined generation + final audit

**Build:** None (side project — ~/games/sip_planner_qwen2.html)
- v2 improvements: system prompt with Q2/Q3 fix rules, HTML split into 4 one-tab stages, exact innerHTML template for Stage 6
- All 9 stages returned finish=stop, no truncation anywhere (~8,173 total tokens, ~731s)
- Q1 FIXED (HTML truncation): splitting into 4 × single-tab prompts → each tab 696–1074 tokens, all complete
- Q2 FIXED (g().value): explicit rule in system prompt → calcLumpsum/calcFull use g() correctly
- Q3 FIXED (if without parens): explicit rule → addExp has `if (expenses.length >= 10) return;`
- Q4 PARTIAL (string literals in oninput): no JSON.stringify, but model used variable VALUES as field names (`e.year` instead of `'year'`)
- New bugs found: missing `let` on container/div/e in strict mode; `let expenses` redeclared across stages
- All post-generation bugs patched via patch_qwen2.py — 7/7 checks pass
- Served at http://192.168.0.130:8080/sip_planner_qwen2.html (24,236 bytes)
- LLManalysis.md updated with v2 results and 2 new patterns (Q5 implicit globals, Q6 cross-stage redeclaration)

**Net result:** v1 needed 3 full stage rewrites. v2 needed only 3 targeted line fixes. Prompt engineering reduced post-generation work by ~80%.
...
- Rebuilt SIP planner generator using qwen3.5:9b with 6 atomic stages (<2k char prompts each)
- `think: false` + `/no_think` system prompt to disable chain-of-thought
- All 6 stages returned finish=stop — no complexity ceiling (contrast: deepseek quit at 1,338 tokens)
- Total: ~8,722 tokens · 704 seconds
- Stages correct: CSS (1,938 tok) ✅, JS helpers (902 tok) ✅, SIP+Stepup calcs (845 tok) ✅
- Stages with bugs: HTML (2,880 tok) — truncated Tab 4, missing .cards wrapper; Lumpsum+Full calcs — g().value double-dereference; Expense UI — if without parens (syntax error), JSON.stringify for string literals
- All bugs patched via patch_qwen_sip.py — 12/12 sanity checks pass
- Served at http://192.168.0.130:8080/sip_planner_qwen.html (25,849 bytes)
- LLManalysis.md updated: Qwen3.5 benchmark table, 4 new failure patterns Q1–Q4, routing table updated, timestamp updated

**Key finding:** Qwen3.5:9b with atomic task decomposition is viable for multi-module webapps. 3/6 stages bug-free without any retry. Remaining 3 have fixable patterns now documented.
...
- deepseek-coder-v2:16b failed to generate the 4-module SIP planner (9,315-char prompt → 1,338 token skeleton, finish=stop early — complexity ceiling confirmed)
- Claude wrote the complete SIP planner directly: 4 tabs (SIP, Step-up SIP, Lumpsum+SIP, Full Planner), dark theme, Chart.js charts, expense management, Indian number format
- Deployed to http://192.168.0.130:8080/sip_planner.html (26KB)
- Post-deploy audit found 2 bugs: (1) mkDs fill color conversion broken for hex strings — `.replace('rgb','rgba')` is a no-op on hex, fills showed solid instead of rgba(x,x,x,0.15); (2) Tab 3 missing `stacked:true` on y-axis — overlapping fills instead of proper stacked area chart
- Both bugs patched via patch_sip.py — `hexToRgba()` helper added, `mkChart()` gains `stacked` param, Tab 3 call updated
- Financial formulas verified correct: SIP annuity-due FV, month-by-month step-up simulation, lumpsum+SIP combined corpus, full planner with year-end expense deduction
- LLManalysis.md to be updated with deepseek webapp complexity ceiling finding (pending)

**Open:** LLManalysis.md webapp-generation section not yet updated

---

## 2026-04-21 — Game benchmark: 5 games audited and rewritten; LLManalysis.md created

**Build:** None (side project — game benchmark, not finpath)
- Session ran full audit of 5 HTML5 games generated by deepseek-coder-v2:16b via Ollama
- All 5 games were broken/unplayable due to model bugs — rewritten from scratch and deployed
- Games live at http://192.168.0.130:8080 (Breakout, Flappy Bird, Snake, Tetris, 2048)
- `C:\dropbox\finpath\LLManalysis.md` — full model audit: 14 recurring bug patterns, strengths, prompt engineering conclusions
- Mirrored to Mac: `~/game-benchmark/LLManalysis.md`
- Key finding: deepseek-coder-v2 gets architecture right but fails on implementation details (const mutable state, listeners inside loop, pipe gap = 0, direction buffer never applied, setInterval not reset on speed change, undefined variables, wrong destructuring)

---

## 2026-04-20 — AAB v34 built and copied to Dropbox

**Build:** `app-release-v34.aab` | versionCode 34, versionName 1.0.2 | 116MB | release-signed
- All C/H/M audit items resolved before this build
- versionCode bumped manually in `android/app/build.gradle` (android/ is gitignored)
- AAB at `C:\dropbox\finpath\app-release-v34.aab`
- **Next:** upload to Play Console internal testing track; test on physical device (P8)

---

## 2026-04-20 — Add TASKS.md with full audit status (commit b427ea1)

**Commit:** `b427ea1` | Branch: `beyondv33`
- Created `kb/TASKS.md` and `C:\dropbox\finpath\TASKS.md` — full todo/done list with fix directions for every audit item (C1–C6, H1–H7, M1–M9, L1–L10, P1–P8)
- Intended as handoff doc for other LLM sessions

---

## 2026-04-20 — Fix UTC date bug in expense PV calc (C4) (commit eaa2754)

**Commit:** `eaa2754` | Branch: `beyondv33` | 70/70 tests pass.
- `new Date(exp.end_date)` and `new Date(exp.start_date)` → `parseDateStr()` in calculator.ts lines 640-641
- `getAge()`, vesting dates, and all other date parsing were already correct
- H4 (create-profile.tsx DOB validation) was already correct too

---

## 2026-04-20 — Seed sipAmount to 0, close H3+M1 (commits 9c4d6f4, 5975eff)

**Commits:** `9c4d6f4` (M4), `5975eff` (M3) | Branch: `beyondv33`
- M3: `useState(10000)` → `useState(0)` for sipAmount + sipAmountDisplay; auto-set fires on first result
- M4: accessibility label `'Goal tab'` → `'Goals tab'`
- M1: already resolved prior to this session (dashboard.tsx line 174 uses `getAge(currentProfile.dob)`)
- H3: already resolved — `FIRE_TARGET_AGES` has `lean:85`, migration v9 normalizes `slim→lean`

---

## 2026-04-20 — Tab accessibility label fix (M4) (commit 9c4d6f4)

**Commit:** `9c4d6f4` | Branch: `beyondv33`
- `tabBarAccessibilityLabel: 'Goal tab'` → `'Goals tab'` in `app/(tabs)/_layout.tsx`
- M1 was already resolved (dashboard.tsx line 174 already uses `getAge(currentProfile.dob)`)
...
- Schema migration v7 column left as harmless dead data (SQLite cannot DROP COLUMN easily)
- `saveGoals` now takes 7 params instead of 8

**Open:** M1 (inline currentAge → getAge()), M3 (sipAmount seed 0), M5 (null sentinel for 0% ROI), play store tasks
